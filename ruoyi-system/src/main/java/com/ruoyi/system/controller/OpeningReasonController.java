    ...
    public class OpeningReasonController extends BaseController
    {
        ...
        @PostMapping("/add")
        @ResponseBody
        public AjaxResult addSave(@RequestBody OpeningReason openingReason)
        {
            System.out.println(openingReason);
        ...
        /**
         * 根据 positionId 查询开仓原因
         */
        @GetMapping("/getByPositionId")
        @ResponseBody
        public AjaxResult getByPositionId(@RequestParam("positionId") Long positionId) {
            OpeningReason openingReason = openingReasonService.selectOpeningReasonByPositionId(positionId);
            if (openingReason != null) {
                return AjaxResult.success(openingReason);
            } else {
                return AjaxResult.error("未找到对应的开仓原因");
            }
        }
